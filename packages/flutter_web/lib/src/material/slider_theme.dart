...
class PaddleSliderValueIndicatorShape extends SliderComponentShape {
  ...
  // Determines the "best" offset to keep the bubble on the screen. The
  // calling code will bound that with the available movement in the paddle
  // shape.
  double _getIdealOffset(
    ...
    // We can just multiply by scale instead of a transform, since we're
    // scaling around (0, 0).
  ...
  void _drawValueIndicator(
    ...
    // The distance between the end of the bottom neck arc and the beginning
    // of the top neck arc. We use this to shrink/expand it based on the scale
