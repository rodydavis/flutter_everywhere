...
import 'package:flutter_web_ui/ui.dart' show window;
...
void main() {
  testWidgets('Slider theme is built by ThemeData',
    ...
    expect(sliderTheme.inactiveTrackColor.value,
        equals(Colors.red.withAlpha(0x3d).value));
  });
  ...
  testWidgets(
    ...
    expect(
        sliderTheme.thumbShape, equals(isInstanceOf<RoundSliderThumbShape>()));
    expect(sliderTheme.valueIndicatorShape,
        equals(isInstanceOf<PaddleSliderValueIndicatorShape>()));
    expect(sliderTheme.showValueIndicator,
        equals(ShowValueIndicator.onlyForDiscrete));
  ...
  testWidgets('SliderThemeData lerps correctly', (WidgetTester tester) async {
    ...
    expect(
        lerp.valueIndicatorTextStyle.color, equals(middleGrey.withAlpha(0xff)));
  });

  testWidgets('Default slider thumb shape draws correctly',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
    ...
    expect(
        sliderBox, paints..circle(color: sliderTheme.thumbColor, radius: 6.0));

    await tester.pumpWidget(buildApp(enabled: false));
    await tester.pumpAndSettle(); // wait for disable animation
    expect(sliderBox,
        paints..circle(color: sliderTheme.disabledThumbColor, radius: 4.0));
  ...
  testWidgets('Default slider value indicator shape draws correctly',
      (WidgetTester tester) async {
    final ThemeData theme = ThemeData(
      platform: TargetPlatform.android,
      primarySwatch: Colors.blue,
