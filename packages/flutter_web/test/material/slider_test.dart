...
void main() {
  ...
  testWidgets('Slider uses the right theme colors for the right components',
    ...
    expect(sliderBox,
        paints..rect(color: customColor1)..rect(color: customColor2));
    expect(sliderBox, paints..circle(color: customColor1));
    expect(sliderBox, isNot(paints..circle(color: sliderTheme.thumbColor)));
    expect(sliderBox,
        isNot(paints..circle(color: sliderTheme.disabledThumbColor)));
  ...
  testWidgets('Value indicator appears when it should',
    ...
    double value = 0.45;
    Widget buildApp(
        {SliderThemeData sliderTheme, int divisions, bool enabled = true}) {
    ...
    await expectValueIndicator(
        isVisible: true, theme: theme, divisions: 3, enabled: true);
    await expectValueIndicator(
    ...
    await expectValueIndicator(
        isVisible: false, theme: theme, divisions: 3, enabled: true);
    await expectValueIndicator(
        isVisible: false, theme: theme, divisions: 3, enabled: false);
